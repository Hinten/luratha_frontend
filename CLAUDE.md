...
npm run test:firestore        # Vitest cloud integration suite (luratha-test) — mandatory for schema/Firebase changes
npm run test:functions:cloud  # Vitest Functions trigger suite — requires Functions deployed to luratha-test
npm run test:e2e              # Playwright E2E against luratha-test (headless Chromium, auto-starts dev server)
...
| `e2e/` | Playwright specs (run against `luratha-test`) |
...
- Cloud integration tests: `src/test/cloud/*.cloud.test.ts` and `src/test/cloud/*.test.ts` (Vitest, node) — run against `luratha-test`
- Cloud Functions trigger tests: `src/test/cloud-functions/*.functions.test.ts` (Vitest, node) — require Functions deployed to `luratha-test`
- E2E: `e2e/*.spec.ts` (Playwright) — runs against `luratha-test`
