...
describe("OrderSummary", () => {
  it("renders subtotal, shipping and total formatted in BRL", () => {
    render(<OrderSummary items={[]} subtotal={200} shippingTotal={20} showItems={false} />);
  ...
  it("renders 'Grátis' when shipping is zero", () => {
    render(<OrderSummary items={[]} subtotal={300} shippingTotal={0} showItems={false} />);
    expect(screen.getByText("Grátis")).toBeInTheDocument();
  });

  it("hides the discount row when discountTotal is 0", () => {
    render(<OrderSummary items={[]} subtotal={100} shippingTotal={10} showItems={false} />);
